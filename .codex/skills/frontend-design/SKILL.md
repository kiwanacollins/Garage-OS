...
## Anti-AI-Slop Rules (read before every build)

GarageOS must not look like it was generated by an AI trained on Tailwind defaults. The following patterns are banned. If any of these appear in output, treat them as bugs and remove them.

### Banned visual patterns
- **No indigo/purple/neon** — GarageOS primary is `#3857A3` (steel blue), accent is `#EE1E24` (red). Never default to Tailwind indigo or any purple gradient.
- **No glowing effects, neon borders, or luminous shadows** — `box-shadow` is elevation only (subtle neutral grey). No colored glow.
- **No oversized rounded corners on layout containers** — `border-radius` max 16px on cards, 8px on controls. Never `rounded-3xl` (24px+) on panels or navbars.
- **No glassmorphism** — no frosted-glass cards, no `backdrop-filter: blur()` outside the topbar, no semi-transparent panels stacked on each other.
- **No animated blobs, floating gradient shapes, or bento-grid layouts** for operational data.
- **No hero → badge → headline → CTA → 3-column feature grid** unless the page is explicitly a marketing page. Operational screens start from the task surface.
- **No repeating identical grid patterns in sequence** — 3-column grid → 3-column grid → 3-column pricing grid is algorithmic. Vary layout structure deliberately between sections.
- **No perfect symmetry everywhere** — intentional visual imbalance creates hierarchy. Primary data is large, supporting data is smaller, metadata is muted. Not every card needs the same weight.
- **No "trusted by logos", testimonials, or FAQ accordions** on operational screens.
- **No soft pastel gradients as page or panel backgrounds** — flat `#F8FAFC` / `#FFFFFF` only. Gradients inside specific KPI tile accents only.
- **No stock 3D illustrations, floating object graphics, or AI-generated people** — real automotive/garage photography or no imagery on operational screens.
- **No excessive hollow whitespace** — density is a feature. Every gap must earn its place by aiding scan, grouping, or separation between distinct sections.

### Banned copy patterns
- No generic headings: "Unlock Productivity", "Streamline Your Experience", "Next-Generation Platform", "Powerful and Flexible", "Everything you need", "Built for teams", "Seamless Digital Experiences", "Empowering businesses with innovative solutions", "Transform your workflow."
- No vague CTAs: "Get Started", "Learn More", "Discover", "Explore." Every button names the exact action: "Register customer", "Generate invoice", "Mark QC passed", "Send signup link."
- No aspirational subheadings inside operational screens. Headings name the task, status, or entity.
- **The copy test:** after reading any heading, you must know what the screen does, who it serves, and why it matters — in under 5 seconds. If you cannot, rewrite it.

### Banned interaction patterns
- No scroll-reveal fade-in on every section. No floating animated elements. No page-load animations that fire without user action.
- No hover animations on static text or decorative icons that add no affordance.
- No toast notifications for every action — async outcomes only (save failed, SMS sent, upload complete). Never for tab switches, filter changes, or row selections.
- No "loading shimmer" on every element — GarageOS analyzer loader only when data is genuinely fetching.
- No empty `onClick` handlers, buttons that go nowhere, or links with `href="#"`.
- No inconsistent UX across pages — icon sets, spacing, and interaction patterns must be uniform across every role screen.

### Banned code patterns
- No comments that describe what code does (`// This is a button`, `// Create user`). Comments only explain non-obvious constraints, workarounds, or business rules.
- No emoji in `console.log` or anywhere in source code.
- No redundant variable declarations, deeply nested conditionals for simple tasks, or copy-pasted Mantine demo code adapted verbatim.
- No unused imports left in after refactoring.
