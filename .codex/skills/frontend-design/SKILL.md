...
- **No glowing effects, neon borders, or luminous shadows** — `box-shadow` is used for elevation only (subtle grey, never colored glow).
...
- **No hero section with a badge + headline + CTA + 3-card pricing grid** unless the page is explicitly a landing/marketing page. Operational screens start from the task surface.
- **No soft pastel gradients as backgrounds** — backgrounds are flat `#F8FAFC` or `#FFFFFF`. Gradients only inside specific KPI tile accents, never full-page.
- **No stock 3D illustrations or floating object graphics** — if imagery is needed, use real automotive/garage photography.
- **No excessive whitespace that feels hollow** — density is a feature. Garage operators need to see information, not breathe.

### Banned copy patterns
- No generic headings: "Unlock Productivity", "Streamline Your Experience", "Next-Generation Platform", "Powerful and Flexible", "Everything you need", "Built for teams".
- No vague CTAs: "Get Started", "Learn More", "Discover", "Explore". Every button says exactly what it does: "Register customer", "Generate invoice", "Mark QC passed", "Send signup link".
- No aspirational subheadings inside operational screens. Headings name the task or surface.

### Banned interaction patterns
- No random hover animations on static text or icons that add no affordance.
- No toast notifications for every action — notifications only for async outcomes (save failed, SMS sent, upload complete). Not for navigation, tab switches, or local state changes.
- No "loading shimmer" on every input or table row — use the GarageOS analyzer loader only when data is genuinely fetching.
